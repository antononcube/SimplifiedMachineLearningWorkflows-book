...
lsBasisImages <- 
  purrr::map(1:nrow(svdH), function(i) { 
    matrix(svdH[i,], ncol = ncol(mandalaImageArrays[[1]]), byrow =F)
  })
...
```{r}
testMandalaSMat <- sparseMatrix( x = as.numeric(testMandalaArray), i = rep_len(1,length(testMandalaArray)), j= 1:length(testMandalaArray))
...
image(matrix(vecRepresentation, ncol = ncol(mandalaImageArrays[[1]]), byrow=F), axes=FALSE, col = gray(0:255/255))
```
